...
"""
1.SR_Module, when sr_ratio == 2, kernel_size of sr1 is changed to 3 (overlap)
2.LargeKernelConv for the Position Attention
"""
...
def SR_Module(in_dim, out_dim, sr_ratio):
    ...
    if sr_ratio > 2:
        ...
        return {'sr1': nn.Conv2d(in_dim, out_dim, kernel_size=3, stride=2, padding=1, bias=False),
                'norm1': nn.LayerNorm(out_dim),
...
class LargeKernelConv(nn.Module):
    def __init__(self, dim, kernel_size=14, dilation=3):
        ...
        self.dw_conv = nn.Conv2d(dim, dim, kernel_size=kernel_dw,
                                 stride=1, padding=padding_dw, groups=dim)
        self.dw_d_conv = nn.Conv2d(dim, dim, kernel_size=kernel_dw_d,
                                   stride=1, padding=padding_dw_d, dilation=dilation, groups=dim)
...
class PA(nn.Module):
    """Pixel Attention -> Depth-wise Conv"""
    def __init__(self, dim, kernel_size, dilation):
        super().__init__()
        self.pa_conv = LargeKernelConv(dim, kernel_size, dilation)
...
class ResnetPatchEmbed(nn.Module):
    def __init__(self, dim=64, with_pos=True):
        super(ResnetPatchEmbed, self).__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(3, dim // 2, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(dim // 2),
            nn.ReLU(),
            nn.Conv2d(dim // 2, dim // 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(dim // 2),
            nn.ReLU(),
        ...
        self.with_pos = with_pos
        if self.with_pos:
            self.pos = PA(dim=dim, kernel_size=14, dilation=3)
