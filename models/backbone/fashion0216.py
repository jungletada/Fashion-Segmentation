...
class FashionFormer(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        imgs = [
            reduce(x, 'B C (k1 H) (k2 W)-> B C H W', reduction='max',k1=4, k2=4),
            reduce(x, 'B C (k1 H) (k2 W)-> B C H W', reduction='max',k1=8, k2=8),
            reduce(x, 'B C (k1 H) (k2 W)-> B C H W', reduction='max',k1=16, k2=16),
            reduce(x, 'B C (k1 H) (k2 W)-> B C H W', reduction='max',k1=32, k2=32),
        ]
        features = []
...
if __name__ == '__main__':
    """
        ...
        Add image projection to patch embedding
        patch_embed1.requires_grad = False
        v = v + local_conv for sr_ration=1
        small: val/mIoU: 0.587570687466615
