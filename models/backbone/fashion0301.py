...
class FashionFormer(nn.Module):
    def __init__(self, img_size=224, num_classes=14, embed_dims=[64, 128, 256, 512],
        ...
        self.semantic_conv = Feat_Semantic(num_cls=num_classes,emb_dim=256,num_layers=3)
    ...
    def combine_semantic_map(self, feats):
        segmap = feats[0]
        for i in range(self.num_stages - 1):
            prev_size = segmap.size()[2:]
            segmap = segmap + F.interpolate(feats[i + 1], size=prev_size, mode='bilinear', align_corners=False)
        segmap = self.semantic_conv(segmap)
        segmap = F.interpolate(segmap, scale_factor=4,mode='bilinear', align_corners=False)
        return segmap
    ...
    def forward(self, x):
        ...
        segmap = self.combine_semantic_map(maps)
        out = self.head(feats)
        return {'output':out, 'aux':segmap}
