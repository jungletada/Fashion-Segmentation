...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            f = f.view(-1, out_h, out_w,self.num_features[i]).permute(0, 3, 1, 2).contiguous()
            features.append(f)
...
if __name__ == '__main__':
    backbone = swin_tiny_patch4_window7_224(num_classes=14)
    x = torch.randn(2,3,224,224)
    out = backbone(x)
    num_param = sum(p.numel() for p in backbone.parameters() if p.requires_grad)
    print("Param (M): {:.2f}".format(num_param / 1e6))
    for i in out:
        print(i.shape)