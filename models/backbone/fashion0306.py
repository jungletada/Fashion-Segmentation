...
class FashionFormer(nn.Module):
    def __init__(self, img_size=224, num_classes=14, embed_dims=[64, 128, 256, 512],
        ...
        self.side_semantic_conv = Feat_Semantic(num_cls=num_classes, emb_dim=256, num_layers=3)
        self.out_cls = nn.Conv2d(num_classes,num_classes,kernel_size=1,stride=1)
    ...
    def side_semantic_maps(self, feats):
        ...
        segmap = self.side_semantic_conv(segmap)
        return segmap
    ...
    def forward(self, x):
        ...
        segmap = self.side_semantic_maps(maps)
        out = self.head(feats) + self.gamma * segmap
