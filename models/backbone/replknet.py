...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
    ...
    if has_large_impl and in_channels == out_channels and out_channels == groups \
            and use_large_impl and stride == 1 \
            and padding == kernel_size // 2 and dilation == 1:
...
class RepLKBlock(nn.Module):
    ...
    def __init__(self, in_channels, dw_channels, block_lk_size,
                 small_kernel, drop_path, small_kernel_merged=False):
        super().__init__()
...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
                 ...
                 small_kernel_merged=False,
                 use_sync_bn=False,
                 norm_intermediate_features=False # for RepLKNet-XL on COCO and ADE20K,
                 # use an extra BN to normalize the intermediate feature maps then feed them into the heads
                 ):
        super().__init__()
        ...
        for stage_idx in range(self.num_stages):
            ...
            if stage_idx < len(layers) - 1: # downsample
    ...
    def forward_features(self, x):
        ...
        if self.out_indices is None: # Just need the final output
            for stage_idx in range(self.num_stages):
                x = self.stages[stage_idx](x)
                if stage_idx < self.num_stages - 1:
                    x = self.transitions[stage_idx](x)
            return x
        else:   # Need the intermediate feature maps
            outs = []
            for stage_idx in range(self.num_stages):
                x = self.stages[stage_idx](x)
